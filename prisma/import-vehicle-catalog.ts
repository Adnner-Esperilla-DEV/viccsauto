    ...
    await prisma.$transaction(async (tx) => {
      for (const duplicate of group) {
        ...
        for (const model of duplicate.models) {
          ...
          if (existing) {
            await tx.productCompatibility.updateMany({
              where: { vehicleModelId: model.id },
              data: { vehicleModelId: existing.id, make: displayName },
            });
            await tx.vehicleImport.updateMany({
              where: { modelId: model.id },
              data: { modelId: existing.id, makeId: target.id, make: displayName },
            });
            await tx.vehicleModel.delete({ where: { id: model.id } });
          } else {
            const slugConflict = await tx.vehicleModel.findFirst({
              where: { makeId: target.id, slug: model.slug },
              select: { id: true },
            });
        ...
        await tx.vehicleImport.updateMany({
          where: { makeId: duplicate.id },
          data: { makeId: target.id, make: displayName },
        });
      ...
      if (target.name !== displayName) {
        ...
        await tx.productCompatibility.updateMany({
          where: { vehicleModel: { makeId: target.id } },
          data: { make: displayName },
        });
...
async function replaceVehicleCatalog(makesBySource: Map<number, string>, modelsBySource: Map<number, VpicModel>) {
  ...
  await prisma.$transaction(
    async (tx) => {
      ...
      const makeIdBySource = new Map(
        storedMakes.flatMap((row) => (row.sourceId == null ? [] : [[row.sourceId, row.id] as const])),
      );
      const modelRows = [...modelsBySource].flatMap(([sourceId, row]) => {
        const makeId = makeIdBySource.get(row.Make_ID);
        return makeId
          ? [{ sourceId, makeId, name: row.Model_Name, slug: `${slugify(row.Model_Name)}-${sourceId}` }]
          : [];
      });
      ...
      await tx.vehicleYear.createMany({
...
async function main() {
  const response = await fetch(VPIC_URL, {
    headers: { Accept: "application/json", "User-Agent": "ViccsAuto vehicle catalog importer" },
  });
  ...
  for (const row of sourceRows) {
    ...
    if (row.Model_ID && row.Model_Name?.trim() && row.Make_ID)
      modelsBySource.set(row.Model_ID, { ...row, Make_Name: row.Make_Name.trim(), Model_Name: row.Model_Name.trim() });
  }
  ...
  const storedMakes = await prisma.vehicleMake.findMany({
    where: { sourceId: { in: [...makesBySource.keys()] } },
