...
function ProductThumbnail({ product, compact = false }: { product: PosProduct; compact?: boolean }) {
  ...
  return product.imageId ? (
    ...
    <span
      className={`${sizeClass} grid shrink-0 place-items-center rounded-xl border border-dashed border-slate-300 bg-slate-50 px-1 text-center text-[10px] font-bold uppercase leading-tight text-slate-400`}
    >
      Sin foto
    </span>
  );
...
export function PosSaleForm({ products }: { products: PosProduct[] }) {
  ...
  const matches = useMemo(() => {
    ...
    return products
      .filter((product) =>
        normalized(`${product.sku} ${product.name} ${product.brand ?? ""} ${product.location}`).includes(term),
      )
      .slice(0, 8);
  }, [products, search]);
    ...
    setLines((current) => {
      ...
      if (existing) {
        return current.map((line) =>
          line.inventoryItemId === product.inventoryItemId
    ...
    <form action={formAction} className="grid gap-7 lg:grid-cols-[minmax(0,1fr)_360px]">
      ...
      <div className="space-y-7">
        <section className="rounded-3xl bg-white p-6 shadow-sm">
          ...
          <p className="mt-1 text-sm text-slate-500">
            Busca por nombre, SKU o marca. Solo aparecen unidades disponibles.
          </p>
                ...
                {matches.length ? (
                  matches.map((product) => (
                    <button
                      ...
                      className="flex w-full items-center gap-4 px-4 py-3 text-left hover:bg-blue-50"
                    >
                      ...
                      <span className="text-right">
                        <b className="block text-blue-700">{money.format(product.price)}</b>
                        <small className="text-slate-500">{product.available} disponibles</small>
                      </span>
                    </button>
                  ))
                ) : (
          ...
          {saleLines.length ? (
            <div className="mt-6 divide-y">
              {saleLines.map(({ product, quantity }) => (
                <article
                  key={product.inventoryItemId}
                  className="grid items-center gap-3 py-4 sm:grid-cols-[64px_minmax(0,1fr)_100px_120px_auto]"
                >
                  ...
                  <input
                    ...
                    value={quantity}
                    onChange={(event) =>
                      changeQuantity(product.inventoryItemId, Number(event.target.value), product.available)
                    }
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      setLines((current) => current.filter((line) => line.inventoryItemId !== product.inventoryItemId))
                    }
            ...
            <div className="mt-6 rounded-2xl bg-slate-50 p-8 text-center text-slate-500">
              Agrega el primer producto a la venta.
            </div>
          )}
        </section>
        ...
        <section className="rounded-3xl bg-white p-6 shadow-sm">
          ...
          <p className="mt-1 text-sm text-slate-500">
            Si dejas correo y teléfono vacíos, se usará Cliente genérico. Con correo o teléfono buscaremos al cliente
            existente y evitaremos duplicarlo.
          </p>
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            ...
            <input
              ...
              type="email"
              autoComplete="email"
              placeholder="Correo para identificarlo"
              className="rounded-xl border p-3"
            />
            <input
              name="customerPhone"
              autoComplete="tel"
              placeholder="Teléfono para identificarlo"
              maxLength={30}
              className="rounded-xl border p-3"
            />
            <textarea
      ...
      <aside className="h-fit rounded-3xl border border-blue-100 bg-gradient-to-br from-white to-blue-50 p-6 text-slate-900 shadow-lg lg:sticky lg:top-6">
        ...
        <div className="mt-5 flex justify-between text-slate-600">
          <span>Unidades</span>
          <span className="font-bold text-slate-900">{lines.reduce((sum, line) => sum + line.quantity, 0)}</span>
        </div>
        <div className="mt-3 flex items-end justify-between border-t border-blue-100 pt-5">
          <span className="font-bold">Total</span>
          <strong className="text-3xl text-blue-700">{money.format(total)}</strong>
        </div>
        ...
        {state.error && (
          <p role="alert" className="mt-5 rounded-xl bg-red-50 p-3 text-sm font-semibold text-red-700">
            {state.error}
          </p>
        )}
        <button
          disabled={pending || lines.length === 0}
          className="mt-6 w-full rounded-full bg-blue-600 px-5 py-3 font-black text-white disabled:cursor-not-allowed disabled:opacity-50"
        >
          {pending ? "Registrando venta…" : "Registrar venta y descontar stock"}
        </button>
        <p className="mt-3 text-center text-xs text-slate-500">
          Venta presencial en Arica: sin despacho ni IVA/tributos adicionales. La operación queda pagada y entregada.
        </p>
      </aside>
    </form>
  );
