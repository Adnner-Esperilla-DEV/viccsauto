  ...
  const matches = useMemo(() => {
    ...
    const filtered = term
      ? customers.filter((customer) =>
          normalize(`${customerName(customer)} ${customer.email} ${customer.phone ?? ""}`).includes(term),
        )
      : customers;
  ...
  return (
    <div className="sm:col-span-2 lg:col-span-3">
      ...
      >
        <IoSearchOutline
          aria-hidden="true"
          className="pointer-events-none absolute left-4 top-4 z-10 h-5 w-5 text-slate-400"
        />
        <input
          ref={inputRef}
          id={`${listId}-input`}
          ...
          >
            {matches.map((customer, index) => (
              ...
              >
                <span className="min-w-0">
                  <strong className="block truncate text-slate-900">{customerName(customer)}</strong>
            ...
            {!matches.length && (
              <div className="px-4 py-8 text-center">
                <p className="font-bold text-slate-700">No encontramos clientes</p>
                <p className="mt-1 text-sm text-slate-500">Prueba con otro nombre, correo o teléfono.</p>
              </div>
            )}
            {matches.length > 0 && customers.length > matches.length && !query && (
              <p className="border-t px-4 py-2 text-xs text-slate-500">Escribe para buscar entre todos los clientes.</p>
            )}
          </div>
        )}
      </div>
