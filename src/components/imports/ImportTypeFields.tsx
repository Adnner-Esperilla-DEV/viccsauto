...
const fuelOptions = ["Gasolina", "Diésel", "Híbrido", "Eléctrico", "GLP", "GNC", "Otro", "Desconocido"].map(
  (value) => ({ value, label: value }),
);
const keyOptions = [
  { value: "NO_KEY", label: "Sin llave" },
  { value: "UNKNOWN", label: "Desconocido" },
  { value: "KEY_PRESENT", label: "Llave disponible" },
];
const titleOptions = [
  { value: "NO_TITLE", label: "Sin título" },
  { value: "PENDING", label: "Pendiente" },
  { value: "RECEIVED", label: "Título recibido" },
];
const field =
...
export function ImportTypeFields({
  makes,
  initialType = "VEHICLE",
  initialVehicle = {},
  initialParts = [],
  ...
  const goodsValueUsd = useMemo(
    () =>
      parts.reduce(
        (totalCents, part) =>
          totalCents +
          Math.round(Math.max(0, Number(part.unitValueUsd) || 0) * 100) * Math.max(0, Number(part.quantity) || 0),
        0,
      ) / 100,
    [parts],
  );
  const finance = getImportFinanceSummary({
    importType: "PARTS",
    valueUsd: goodsValueUsd,
    towingCostUsd: 0,
    oceanFreightUsd: 0,
    shippingCostUsd: Number(shippingCostUsd) || 0,
    logisticsServiceUsd: Number(logisticsServiceUsd) || 0,
    otherChargesUsd: Number(otherChargesUsd) || 0,
    ...
    <>
          ...
          {(
            [
              { value: "VEHICLE", label: "Vehículo", description: "VIN, datos técnicos, título y llaves." },
              {
                ...
                description: "Una o varias piezas con compatibilidad vehicular.",
              },
            ] as const
          ).map((option) => (
            <label
              key={option.value}
              className={`rounded-2xl border-2 p-4 ${importType === option.value ? "border-blue-600 bg-blue-50" : "border-slate-200"}`}
            >
              <input
                type="radio"
                name="importType"
      ...
      {importType === "VEHICLE" ? (
          ...
          <section className="grid gap-4 rounded-3xl bg-white p-6 shadow-sm sm:grid-cols-2 lg:grid-cols-3">
            ...
            <label className={label}>
              ...
              <input
                ...
                min="1900"
                max="2100"
                defaultValue={initialVehicle.year ?? new Date().getFullYear()}
                className={field}
              />
            </label>
            <ImportVehicleSelector
              makes={makes}
              initialMake={initialVehicle.make}
              initialModel={initialVehicle.model}
            />
            <label className={label}>
              Color
              <input required name="color" defaultValue={initialVehicle.color} className={field} />
            </label>
            <label className={label}>
              ...
              <input
                ...
                step="0.01"
                name="weightKg"
                defaultValue={initialVehicle.weightKg ?? ""}
                className={field}
              />
            </label>
            <StyledSelect
              name="fuel"
              label="Combustible"
              defaultValue={initialVehicle.fuel ?? "Desconocido"}
              options={fuelOptions}
            />
          </section>
          <ImportFinanceFields
            initialValueUsd={initialFinance.valueUsd}
            initialTowingCostUsd={initialFinance.towingCostUsd}
            initialOceanFreightUsd={initialFinance.oceanFreightUsd}
            initialOtherChargesUsd={initialFinance.otherChargesUsd}
            initialPaidAmountUsd={initialFinance.paidAmountUsd}
          />
          <input type="hidden" name="shippingCostUsd" value="0" />
          <input type="hidden" name="logisticsServiceUsd" value="0" />
          <input type="hidden" name="partsJson" value="[]" />
          <section className="grid gap-4 rounded-3xl bg-white p-6 shadow-sm sm:grid-cols-2 lg:grid-cols-3">
            <h2 className="text-xl font-black sm:col-span-2 lg:col-span-3">Título y llaves</h2>
            <StyledSelect
              name="keyStatus"
              label="Llaves"
              defaultValue={initialVehicle.keyStatus ?? "UNKNOWN"}
              options={keyOptions}
            />
            <StyledSelect
              name="titleStatus"
              label="Título"
              defaultValue={initialVehicle.titleStatus ?? "PENDING"}
              options={titleOptions}
            />
            <label className={label}>
              Número de título <span className="font-normal text-slate-400">(opcional)</span>
              <input name="titleNumber" defaultValue={initialVehicle.titleNumber ?? ""} className={field} />
            </label>
            <label className={label}>
              Estado / jurisdicción <span className="font-normal text-slate-400">(opcional)</span>
              <input name="titleState" defaultValue={initialVehicle.titleState ?? ""} className={field} />
            </label>
            <label className={label}>
              Schedule B / HTS <span className="font-normal text-slate-400">(opcional)</span>
              <input name="scheduleB" defaultValue={initialVehicle.scheduleB ?? ""} className={field} />
            </label>
          </section>
        </>
      ) : (
        <>
          ...
          <input type="hidden" name="valueUsd" value={goodsValueUsd.toFixed(2)} />
          <input type="hidden" name="towingCostUsd" value="0" />
          <input type="hidden" name="oceanFreightUsd" value="0" />
          <section className="rounded-3xl bg-white p-6 shadow-sm">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <h2 className="text-xl font-black">Repuestos o autopartes</h2>
                <p className="mt-1 text-sm text-slate-500">
                  Registra cada pieza y el vehículo con el que es compatible.
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="mt-5 grid gap-5">
              {parts.map((part, index) => (
                <PartFields
                  ...
                  index={index}
                  makes={makes}
                  update={(changes) => updatePart(part.id, changes)}
                  remove={
                    parts.length > 1
          ...
          <section className="overflow-hidden rounded-3xl border-2 border-blue-200 bg-gradient-to-br from-blue-50 via-white to-emerald-50 shadow-sm">
            <div className="border-b border-blue-100 px-6 py-5">
              ...
              <h2 className="mt-1 text-2xl font-black">Resumen financiero de la importación</h2>
              <p className="mt-2 text-sm text-slate-600">
                El total incluye el valor de las piezas, el envío, el servicio logístico y otros cargos. Todo se
                registra en USD.
              </p>
            </div>
            <div className="grid gap-4 p-6 sm:grid-cols-2 lg:grid-cols-5">
              <label className={financeLabel}>
                ...
                <input readOnly value={goodsValueUsd.toFixed(2)} className={`${field} bg-slate-100`} />
              </label>
              <label className={financeLabel}>
                Costo de envío (USD)
                <input
                  ...
                  onChange={(event) => setShippingCostUsd(event.target.value)}
                  className={field}
                />
              </label>
              <label className={financeLabel}>
                Servicio logístico (USD)
                <input
                  required
                  type="number"
                  min="0"
                  step="0.01"
                  name="logisticsServiceUsd"
                  value={logisticsServiceUsd}
                  onChange={(event) => setLogisticsServiceUsd(event.target.value)}
                  className={field}
                />
              </label>
              <label className={financeLabel}>
            ...
            <div className="grid gap-3 border-t border-blue-100 bg-white/70 p-6 sm:grid-cols-3">
              <FinanceCard label="Total de importación" value={finance.totalUsd} />
              <FinanceCard label="Cancelado" value={finance.paidAmountUsd} />
              <FinanceCard label="Saldo pendiente" value={finance.balanceUsd} />
            </div>
          </section>
        </>
      )}
...
function PartFields({
  ...
  index: number;
  makes: Option[];
  update: (changes: Partial<ImportedPartDraft>) => void;
  remove?: () => void;
}) {
  return (
    <fieldset className="rounded-2xl border border-slate-200 p-5">
      <div className="flex items-center justify-between">
        <legend className="font-black text-blue-800">Repuesto {index + 1}</legend>
        {remove && (
          <button type="button" onClick={remove} className="text-sm font-bold text-red-700">
            Eliminar
          </button>
        )}
      </div>
      <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <label className={label}>
          ...
          <input
            ...
            onChange={(event) => update({ partNumber: event.target.value.toUpperCase() })}
            className={field}
          />
        </label>
        <label className={label}>
          Marca del repuesto <span className="font-normal text-slate-400">(opcional)</span>
          <input
            maxLength={80}
            value={part.partBrand}
            onChange={(event) => update({ partBrand: event.target.value })}
            className={field}
          />
        </label>
        <label className={label}>
        ...
        <label className={label}>
          ...
          <input
            ...
            min="0"
            step="0.01"
            value={part.unitValueUsd}
            onChange={(event) => update({ unitValueUsd: event.target.value })}
            className={field}
          />
        </label>
        <label className={label}>
          Peso unitario (kg) <span className="font-normal text-slate-400">(opcional)</span>
          <input
            type="number"
            min="0.01"
            step="0.01"
        ...
        />
        <label className={label}>
          Motor / versión <span className="font-normal text-slate-400">(opcional)</span>
          <input
            maxLength={80}
            value={part.engine}
        ...
        <div className="rounded-xl bg-slate-50 p-4">
          ...
          <strong className="mt-1 block text-lg text-blue-800">
            {formatUsd((Number(part.unitValueUsd) || 0) * (Number(part.quantity) || 0))}
          </strong>
        </div>
      </div>
    </fieldset>
...
function FinanceCard({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-2xl border border-blue-200 bg-blue-50 p-4 text-blue-800">
      ...
      <strong className="mt-1 block text-xl font-black">{formatUsd(value)}</strong>
    </div>
  );
}
