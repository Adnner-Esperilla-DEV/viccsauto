...
export function ImportVehicleSelector({
  ...
  const [loading, setLoading] = useState(Boolean(initialMake));
  ...
  return (
    <div className="grid gap-4 sm:col-span-2 sm:grid-cols-2 lg:col-span-2">
      {includeHiddenInputs && (
        <>
      ...
      <div className="relative">
        <label htmlFor={makeInputId} className="text-sm font-bold text-slate-700">
          {makeLabel}
        </label>
        <div className="relative mt-2">
          ...
          <input
            ...
            aria-controls={`${makeInputId}-list`}
            ...
            onKeyDown={(event) => {
              ...
              if (event.key === "Escape") setMakeOpen(false);
            }}
            onChange={(event) => {
              ...
              setLoadError(false);
              setMakeOpen(true);
              onSelectionChange?.({ make: null, model: null });
            }}
          />
        </div>
        ...
        {!selectedMake && makeQuery && (
          <p className="mt-1 text-xs text-amber-700">Selecciona una marca de las sugerencias.</p>
        )}
      </div>
      <div className="relative">
        <label htmlFor={modelInputId} className="text-sm font-bold text-slate-700">
          {modelLabel}
        </label>
        <div className="relative mt-2">
          <IoSearchOutline
          ...
          <input
            ...
            aria-controls={`${modelInputId}-list`}
            placeholder={
              loading
                ? "Cargando modelos…"
                : selectedMake
            ...
            onChange={(event) => {
              ...
              setSelectedModel(null);
              setModelOpen(true);
              onSelectionChange?.({ make: selectedMake, model: null });
            }}
          />
        </div>
        {modelOpen && modelQuery.trim() && selectedMake && !loading && !loadError && (
          <Options
            id={`${modelInputId}-list`}
        ...
        {loadError && (
          <p className="mt-1 text-xs text-red-700">No se pudieron cargar los modelos. Vuelve a seleccionar la marca.</p>
        )}{" "}
        {!selectedModel && modelQuery && !loading && (
          <p className="mt-1 text-xs text-amber-700">Selecciona un modelo de las sugerencias.</p>
        )}
      </div>
    </div>
  );
