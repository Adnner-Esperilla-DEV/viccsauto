...
export function ImportVehicleSelector({ makes, initialMake = null, initialModel = null }: { makes: Option[]; initialMake?: Option | null; initialModel?: Option | null }) {
  ...
  return <div className="grid gap-4 sm:col-span-2 sm:grid-cols-2 lg:col-span-2">
    <input type="hidden" name="makeId" value={selectedMake?.id ?? ""}/><input type="hidden" name="modelId" value={selectedModel?.id ?? ""}/>
    <input type="hidden" name="make" value={selectedMake?.name ?? ""}/><input type="hidden" name="model" value={selectedModel?.name ?? ""}/>
    <div className="relative"><label htmlFor="import-make" className="text-sm font-bold text-slate-700">Marca</label><div className="relative mt-2"><IoSearchOutline className="pointer-events-none absolute left-4 top-4 h-5 w-5 text-slate-400" aria-hidden="true"/><input ref={makeInput} id="import-make" required value={makeQuery} autoComplete="off" role="combobox" aria-expanded={makeOpen && Boolean(makeQuery.trim())} aria-controls="import-make-list" placeholder="Escribe, por ejemplo: Ford" className={inputClass} onFocus={() => setMakeOpen(true)} onBlur={() => setMakeOpen(false)} onKeyDown={(event) => { if (event.key === "Enter" && makeOpen && matchingMakes[0]) { event.preventDefault(); chooseMake(matchingMakes[0]); } if (event.key === "Escape") setMakeOpen(false); }} onChange={(event) => { event.currentTarget.setCustomValidity("Selecciona una marca de la lista."); setMakeQuery(event.target.value); setSelectedMake(null); setSelectedModel(null); setModelQuery(""); setModels([]); setMakeOpen(true); }}/></div>
      {makeOpen && makeQuery.trim() && <Options id="import-make-list" options={matchingMakes} selectedId={selectedMake?.id} empty="No encontramos esa marca." choose={chooseMake}/>} {!selectedMake && makeQuery && <p className="mt-1 text-xs text-amber-700">Selecciona una marca de las sugerencias.</p>}
    </div>
    <div className="relative"><label htmlFor="import-model" className="text-sm font-bold text-slate-700">Modelo</label><div className="relative mt-2"><IoSearchOutline className="pointer-events-none absolute left-4 top-4 h-5 w-5 text-slate-400" aria-hidden="true"/><input ref={modelInput} id="import-model" required disabled={!selectedMake || loading || loadError} value={modelQuery} autoComplete="off" role="combobox" aria-expanded={modelOpen && Boolean(modelQuery.trim())} aria-controls="import-model-list" placeholder={loading ? "Cargando modelos…" : selectedMake ? `Buscar modelo de ${selectedMake.name}` : "Primero selecciona la marca"} className={inputClass} onFocus={() => setModelOpen(true)} onBlur={() => setModelOpen(false)} onKeyDown={(event) => { if (event.key === "Enter" && modelOpen && matchingModels[0]) { event.preventDefault(); chooseModel(matchingModels[0]); } if (event.key === "Escape") setModelOpen(false); }} onChange={(event) => { event.currentTarget.setCustomValidity("Selecciona un modelo de la lista."); setModelQuery(event.target.value); setSelectedModel(null); setModelOpen(true); }}/></div>
      {modelOpen && modelQuery.trim() && selectedMake && !loading && !loadError && <Options id="import-model-list" options={matchingModels} selectedId={selectedModel?.id} empty={`No encontramos ese modelo para ${selectedMake.name}.`} choose={chooseModel}/>} {loadError && <p className="mt-1 text-xs text-red-700">No se pudieron cargar los modelos. Vuelve a seleccionar la marca.</p>} {!selectedModel && modelQuery && !loading && <p className="mt-1 text-xs text-amber-700">Selecciona un modelo de las sugerencias.</p>}
    </div>
  </div>;
}
