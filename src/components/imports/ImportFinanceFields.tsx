  ...
  return (
    <section className="overflow-hidden rounded-3xl border-2 border-blue-200 bg-gradient-to-br from-blue-50 via-white to-emerald-50 shadow-sm">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-t border-blue-100 px-6 py-4">
        <p className="text-sm font-semibold text-slate-600">Puedes registrar un abono parcial o marcar el pago completo.</p>
        <button
          type="button"
...
function FinanceCard({ label, value, tone }: { label: string; value: number; tone: "blue" | "green" | "amber" }) {
  ...
  return <div className={`rounded-2xl border p-4 ${tones[tone]}`}><span className="block text-xs font-bold uppercase tracking-wider opacity-70">{label}</span><strong className="mt-1 block text-xl font-black" aria-live="polite">{formatUsd(value)}</strong></div>;
}
