...
export function ImportFinanceSummary({
  importType = "VEHICLE",
  valueUsd,
  towingCostUsd,
  oceanFreightUsd,
  shippingCostUsd = 0,
  logisticsServiceUsd = 0,
  otherChargesUsd = 0,
  paidAmountUsd,
}: {
    ...
    <section className="overflow-hidden rounded-3xl border-2 border-blue-200 bg-white shadow-lg shadow-blue-950/5">
      ...
      <div className="grid gap-3 border-t border-blue-100 bg-blue-50/60 p-6 sm:grid-cols-2">
        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-emerald-800">
          <span className="text-xs font-bold uppercase tracking-wider">Monto cancelado</span>
          <strong className="mt-1 block text-2xl font-black">{formatUsd(summary.paidAmountUsd)}</strong>
        </div>
        <div
          className={`rounded-2xl border p-4 ${summary.balanceUsd > 0 ? "border-amber-200 bg-amber-50 text-amber-800" : "border-emerald-200 bg-emerald-50 text-emerald-800"}`}
        >
          <span className="text-xs font-bold uppercase tracking-wider">Saldo pendiente</span>
          <strong className="mt-1 block text-2xl font-black">{formatUsd(summary.balanceUsd)}</strong>
        </div>
      </div>
      <p className="border-t border-blue-100 px-6 py-3 text-center text-xs font-semibold text-slate-500">
        {importType === "PARTS"
          ? "El total incluye los repuestos, el envío, el servicio logístico y otros cargos."
          : "El total incluye grúa, flete marítimo y otros cargos. El valor del vehículo se muestra por separado."}{" "}
...
function CostRow({ label, value, strong = false }: { label: string; value: number; strong?: boolean }) {
  return (
    <div
      className={`flex items-center justify-between gap-4 border-b border-slate-100 py-2 ${strong ? "text-blue-800" : "text-slate-700"}`}
    >
      <span className={strong ? "font-black" : "text-sm"}>{label}</span>
      <span className={strong ? "text-lg font-black" : "font-bold"}>{formatUsd(value)}</span>
    </div>
  );
}
