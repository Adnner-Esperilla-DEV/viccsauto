...
export function ImportDatePicker({ initialValue = "" }: { initialValue?: string }) {
  ...
  return <div className="relative" onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setOpen(false); }}>
    <span className="text-sm font-bold text-slate-700">Fecha de recepción <span className="font-normal text-slate-400">(opcional)</span></span>
    <input type="hidden" name="receivedDate" value={value}/>
    ...
    {open && <div role="dialog" aria-label="Seleccionar fecha de recepción" className="absolute left-0 top-full z-50 mt-2 w-[min(22rem,calc(100vw-3rem))] rounded-3xl border border-slate-200 bg-white p-4 shadow-2xl">
