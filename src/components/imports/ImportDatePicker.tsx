...
export function ImportDatePicker({
  ...
  return (
    ...
    >
      ...
      <div
        className={`mt-2 flex overflow-hidden rounded-xl border bg-white transition focus-within:border-blue-600 focus-within:ring-4 focus-within:ring-blue-100 ${open ? "border-blue-600 ring-4 ring-blue-100" : "border-slate-300"}`}
      >
        <button
          type="button"
        ...
        >
          <IoCalendarOutline aria-hidden="true" className="h-5 w-5 shrink-0 text-blue-700" />
        ...
        {value && (
          <button
            ...
            className="px-3 text-slate-400 hover:bg-slate-50 hover:text-red-600"
          >
      ...
      {open && (
        <div
          ...
          className="absolute left-0 top-full z-50 mt-2 w-[min(22rem,calc(100vw-3rem))] rounded-3xl border border-slate-200 bg-white p-4 shadow-2xl"
        >
          <div className="flex items-center justify-between">
            <button
              ...
              className="grid h-10 w-10 place-items-center rounded-xl text-slate-600 hover:bg-blue-50 hover:text-blue-700"
            >
          ...
          <div className="mt-3 grid grid-cols-7 text-center">
            {weekdays.map((day) => (
              <span key={day} className="py-2 text-xs font-black uppercase text-slate-400">
                {day}
              </span>
            ))}
            {days.map((day, index) =>
              day ? (
                <button
                  ...
                  type="button"
                  onClick={() => select(day)}
                  className={`mx-auto grid h-10 w-10 place-items-center rounded-xl text-sm font-semibold transition ${value === isoDate(visibleYear, visibleMonth, day) ? "bg-blue-700 text-white shadow-md" : isoDate(visibleYear, visibleMonth, day) === isoDate(today.getFullYear(), today.getMonth(), today.getDate()) ? "bg-blue-50 text-blue-800 ring-1 ring-blue-200" : "text-slate-700 hover:bg-slate-100"}`}
                >
                  {day}
                </button>
              ) : (
          ...
          <div className="mt-3 flex items-center justify-between border-t pt-3">
            <button
              ...
              onClick={() => {
                ...
                selectToday(today, setValue, setOpen);
              }}
              className="rounded-xl px-3 py-2 text-sm font-bold text-blue-700 hover:bg-blue-50"
            >
              Hoy
            </button>
            {value && (
