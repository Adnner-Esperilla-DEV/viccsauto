...
function SaveButton() {
  ...
  return (
    <button
      disabled={pending}
      className="rounded-xl bg-blue-700 px-6 py-3 font-bold text-white hover:bg-blue-800 disabled:cursor-wait disabled:opacity-60"
    >
    ...
    >
      ...
      <input
        ...
        value={JSON.stringify(
          images.map((image) => (image.kind === "existing" ? { id: image.id } : { data: image.data })),
        )}
      />
      {serverError && (
        <p
      ...
      <label className="grid gap-2 text-sm font-bold text-slate-700">
        Categoría
        <select required name="categoryId" defaultValue={product.categoryId} className={inputClass}>
      ...
      <label className="grid gap-2 text-sm font-bold text-slate-700">
        Precio anterior <span className="font-normal text-slate-400">(opcional)</span>
        <input
          type="number"
          name="compareAtPrice"
          min={1}
      ...
      <div className="sm:col-span-2">
        <div className="flex items-end justify-between">
          <span className="text-sm font-bold text-slate-700">Imágenes</span>
        ...
        {images.length > 0 && (
          <div className="mt-3 grid gap-3 sm:grid-cols-3 lg:grid-cols-5">
            {images.map((image, index) => (
              <article
                key={image.kind === "existing" ? image.id : `${image.name}-${index}`}
                className={`overflow-hidden rounded-2xl border bg-white ${index === 0 ? "border-blue-500 ring-2 ring-blue-100" : "border-slate-200"}`}
              >
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={image.url} alt={`Imagen ${index + 1}`} className="aspect-[4/3] w-full object-contain" />
                <div className="border-t border-slate-100 p-2">
                  <span className={`text-[11px] font-bold ${index === 0 ? "text-blue-700" : "text-slate-500"}`}>
        ...
        {images.length < MAX_PRODUCT_IMAGES && (
          <label className="mt-3 flex cursor-pointer items-center justify-center gap-3 rounded-2xl border-2 border-dashed border-slate-300 px-5 py-6 text-center hover:border-blue-500 hover:bg-blue-50">
            ...
            <span>
              <b className="block text-sm">{processing ? "Optimizando imágenes..." : "Agregar imágenes"}</b>
              <small className="text-slate-500">Conversión automática a WEBP 4:3</small>
            </span>
            <input
              ref={inputRef}
              type="file"
      ...
      <label className="flex items-start gap-3 rounded-2xl border border-blue-100 bg-blue-50 p-4 sm:col-span-2">
        ...
        <span>
          ...
          <span className="mt-1 block text-xs font-normal text-slate-600">Requiere al menos una imagen.</span>
        </span>
      </label>
