...
export function ProductCompatibilityModal({ product, makes, years, items, closeHref, listPage, listQuery, error, success }: ProductCompatibilityModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-slate-950/55 p-4 backdrop-blur-sm sm:p-8" onMouseDown={(event) => { if (event.currentTarget === event.target) close(); }}>
      <section role="dialog" aria-modal="true" aria-labelledby={`compatibility-${product.id}`} className="my-auto w-full max-w-4xl overflow-hidden rounded-3xl bg-white shadow-2xl">
        <header className="flex items-start justify-between gap-6 border-b border-slate-200 px-6 py-5 sm:px-8">
          <div>
            ...
            <h2 id={`compatibility-${product.id}`} className="mt-1 text-2xl font-black text-slate-950">{product.name}</h2>
            <p className="mt-1 text-sm font-normal text-slate-500">{product.sku} · {items.length} registros</p>
          </div>
          <button type="button" onClick={close} aria-label="Cerrar modal" className="grid h-10 w-10 shrink-0 place-items-center rounded-full text-slate-500 hover:bg-slate-100 hover:text-slate-900">
            <IoCloseOutline className="h-6 w-6" />
          </button>
        </header>

        <div className="max-h-[calc(100vh-10rem)] overflow-y-auto px-6 py-6 sm:px-8">
          {error && <p role="alert" className="mb-5 rounded-2xl bg-red-50 p-4 text-sm font-semibold text-red-700">{error}</p>}
          {success && <p className="mb-5 rounded-2xl bg-emerald-50 p-4 text-sm font-semibold text-emerald-800">Compatibilidad actualizada correctamente.</p>}

          <form action={addProductCompatibilityAction} className="grid gap-4 rounded-2xl border border-slate-200 p-5 md:grid-cols-2">
            ...
            <button className="rounded-xl bg-blue-700 p-3 font-bold text-white md:col-span-2">Agregar compatibilidad</button>
          </form>
          ...
          <div className="mt-7">
            ...
            ) : <p className="mt-4 rounded-2xl bg-amber-50 p-4 text-amber-800">Este producto aún no tiene compatibilidades.</p>}
          </div>
        </div>
      </section>
    </div>
