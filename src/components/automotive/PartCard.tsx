      ...
      <Link href={`/product/${product.slug}`} className="block">
        ...
        <div className="space-y-3 p-5">
          <div className="flex items-center justify-between gap-3 text-xs font-semibold uppercase tracking-wider text-slate-500">
            <span>{product.brand ?? "Marca no especificada"}</span>
            <span>{product.sku}</span>
          </div>
          <h3 className="min-h-12 text-lg font-bold leading-tight text-slate-900 group-hover:text-blue-700">
            {product.name}
          </h3>
          <p className="line-clamp-2 text-sm text-slate-600">{product.shortDescription}</p>
          <div className="flex items-end justify-between gap-3 pt-2">
            <div>
              <strong className="block text-xl text-slate-950">{formatPrice(product.price)}</strong>
              {product.compareAtPrice && (
                <span className="text-xs text-slate-400 line-through">{formatPrice(product.compareAtPrice)}</span>
              )}
            </div>
            <span className="flex items-center gap-1 text-xs font-medium text-emerald-700">
              <IoCheckmarkCircle /> {product.stock} disponibles
            </span>
          </div>
        </div>
      </Link>
