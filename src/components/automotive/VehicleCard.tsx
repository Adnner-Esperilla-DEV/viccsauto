    ...
    <article className="overflow-hidden rounded-3xl bg-slate-950 text-white shadow-lg">
      <Link href={`/vehicle/${vehicle.slug}`} className="group block">
        {vehicle.image ? (
          ...
          <img
            src={vehicle.image}
            alt={`Portada de ${vehicle.year} ${vehicle.make} ${vehicle.model}`}
            className="aspect-[16/9] w-full bg-white object-cover transition duration-300 group-hover:scale-[1.02]"
          />
        ) : (
          <div className="flex aspect-[16/9] items-center justify-center bg-gradient-to-br from-blue-800 via-blue-950 to-slate-950">
            <IoCarSportOutline className="h-24 w-24 text-blue-300 transition group-hover:scale-110" />
          </div>
        )}
        <div className="space-y-4 p-6">
          ...
          <div className="flex gap-5 text-sm text-slate-300">
            <span className="flex items-center gap-1">
              <IoSpeedometerOutline /> {vehicle.mileage.toLocaleString("es-CL")} km
            </span>
            <span className="flex items-center gap-1">
              <IoLocationOutline /> {vehicle.location}
            </span>
          </div>
