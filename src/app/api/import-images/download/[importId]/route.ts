...
export async function GET(_request: Request, { params }: { params: Promise<{ importId: string }> }) {
  ...
  const vehicleImport = await db.vehicleImport.findUnique({ where: { id: (await params).importId }, select: { customerId: true, vin: true, year: true, make: true, model: true, images: { orderBy: { position: "asc" }, select: { filename: true, data: true, storageKey: true } } } });
  ...
  let images;
  try {
    images = await Promise.all(vehicleImport.images.map(async (image) => ({
