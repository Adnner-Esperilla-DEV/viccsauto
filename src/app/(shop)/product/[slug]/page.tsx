...
export default async function ProductPage({
  ...
  return (
    <main className="mx-auto max-w-7xl px-6 py-12">
      ...
      {query.error && (
        <p role="alert" className="mb-6 rounded-2xl bg-red-50 p-4 text-red-700">
      ...
      <div className="grid gap-12 lg:grid-cols-2">
        ...
        <div>
          <span className="text-sm font-bold uppercase tracking-widest text-blue-700">
            {product.brand ?? "Sin marca especificada"} · {product.sku}
          </span>
          ...
          {product.stock > 0 && (
            <form action={addToCartAction} className="mt-7 flex gap-3">
              ...
              <input
                ...
                defaultValue={1}
                min={1}
                max={Math.min(product.stock, 25)}
                className="w-20 rounded-full border px-4"
              />
          ...
          <section className="mt-10 rounded-3xl border border-slate-200 bg-white p-6">
            ...
            {product.compatibility.map((item) => (
              ...
              >
                <span>
                  <b>Vehículo:</b> {item.make} {item.model}
                </span>
                <span>
                  <b>Años:</b> {item.years}
                </span>
                {item.engine && (
                  <span>
                    <b>Motor:</b> {item.engine}
                  </span>
                )}
              </div>
            ))}
            <p className="mt-5 text-xs text-slate-500">
              Códigos OEM: {product.oemCodes.join(", ")}. Verifica siempre con VIN o muestra.
            </p>
          </section>
        </div>
      </div>
