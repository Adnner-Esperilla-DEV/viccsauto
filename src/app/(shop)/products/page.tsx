  ...
  return (
    <main className="mx-auto max-w-7xl px-6 py-14">
      <div className="max-w-3xl">
        ...
        <p className="mt-4 text-slate-600">Selecciona la marca y el modelo de tu vehículo. Las opciones se generan desde los repuestos que realmente están disponibles.</p>
      </div>
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm font-semibold text-slate-600">{filtered.length} {filtered.length === 1 ? "repuesto disponible" : "repuestos disponibles"}</p>
      </div>

      {filtered.length ? (
        <div className="grid gap-7 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((product) => <PartCard key={product.id} product={product} />)}
        </div>
      ) : (
