...
export default async function CheckoutPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  const items = summary.items.map((item) => ({ id: item.id, name: item.product.name, quantity: item.quantity, unitPrice: item.unitPrice }));
  ...
  return <main className="mx-auto max-w-6xl px-6 py-14">
    ...
    {query.error && <p role="alert" className="mt-6 rounded-2xl bg-red-50 p-4 text-red-700">No pudimos crear el pedido. Revisa los datos, el destino y la disponibilidad.</p>}
