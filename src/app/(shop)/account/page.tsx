...
export default async function AccountPage() {
  ...
  return <main className="mx-auto max-w-5xl px-6 py-12"><p className="font-bold uppercase tracking-widest text-blue-700">Mi cuenta</p><h1 className="mt-2 text-4xl font-black">Hola, {user.firstName}</h1><p className="mt-2 text-slate-600">Consulta tus compras y sigue tus vehículos importados.</p><div className="mt-8 grid gap-5 sm:grid-cols-2"><Link href="/imports" className="rounded-3xl border bg-white p-7 shadow-sm transition hover:border-blue-300"><p className="text-sm font-bold uppercase text-blue-700">Logística</p><h2 className="mt-2 text-2xl font-black">Mis importaciones</h2><p className="mt-2 text-slate-500">{imports} {imports === 1 ? "vehículo asignado" : "vehículos asignados"}</p></Link><Link href="/orders" className="rounded-3xl border bg-white p-7 shadow-sm transition hover:border-blue-300"><p className="text-sm font-bold uppercase text-blue-700">Compras</p><h2 className="mt-2 text-2xl font-black">Mis pedidos</h2><p className="mt-2 text-slate-500">{orders} {orders === 1 ? "pedido" : "pedidos"}</p></Link></div></main>;
}
