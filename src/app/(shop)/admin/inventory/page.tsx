...
export default async function InventoryAdminPage({ searchParams }: InventoryAdminPageProps) {
  ...
  const [items, historyItem] = await Promise.all([
    ...
    query.history ? db.inventoryItem.findUnique({
  ...
  const movements = movementWhere ? await db.inventoryMovement.findMany({
    where: movementWhere,
  ...
  const administratorIds = [...new Set(movements.flatMap((movement) => movement.reference?.startsWith("ADMIN:") ? [movement.reference.slice(6)] : []))];
  const administrators = administratorIds.length ? await db.user.findMany({
    where: { id: { in: administratorIds } },
    select: { id: true, firstName: true, lastName: true },
  }) : [];
  const administratorNames = new Map(administrators.map((administrator) => [administrator.id, `${administrator.firstName} ${administrator.lastName}`]));
  ...
  return (
    <main className="mx-auto w-full max-w-[1500px] px-4 py-10 sm:px-6 lg:px-8">
      ...
      {query.ok === "adjusted" && <p className="mt-6 rounded-2xl border border-emerald-100 bg-emerald-50 p-4 text-sm font-semibold text-emerald-800">Inventario ajustado correctamente.</p>}
      {query.error === "invalid" && <p role="alert" className="mt-6 rounded-2xl border border-red-100 bg-red-50 p-4 text-sm font-semibold text-red-700">Revisa las unidades y escribe un motivo válido.</p>}
      {query.error === "negative" && <p role="alert" className="mt-6 rounded-2xl border border-red-100 bg-red-50 p-4 text-sm font-semibold text-red-700">El ajuste no puede dejar menos unidades que las actualmente reservadas.</p>}

      <div className="mt-8 space-y-4">
        {items.map((item) => {
          const available = item.quantity - item.reserved;
          return (
            <article key={item.id} className="grid items-center gap-5 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm md:grid-cols-[minmax(0,1fr)_auto] xl:grid-cols-[minmax(260px,1fr)_auto_440px]">
              <div className="min-w-0">
                <b className="block truncate text-slate-950">{item.product.name}</b>
                <small className="block text-slate-500">{item.product.sku} · {item.location.name}</small>
                <Link href={historyHref(page, item.id)} className="mt-2 inline-flex text-xs font-bold text-blue-700 hover:underline">Ver historial</Link>
              </div>
              <div className={`whitespace-nowrap rounded-xl px-3 py-2 text-sm font-black ${available <= item.product.lowStockAt ? "bg-red-50 text-red-700" : "bg-emerald-50 text-emerald-700"}`}>
                {available} disponibles
              </div>
              <form action={adjustInventoryAction} className="grid gap-2 sm:grid-cols-[120px_minmax(0,1fr)_auto] md:col-span-2 xl:col-span-1">
                <input type="hidden" name="itemId" value={item.id} />
                <input type="hidden" name="returnPage" value={page} />
                <input required type="number" name="delta" placeholder="+/- unidades" aria-label={`Ajuste de unidades para ${item.product.name}`} className="min-w-0 rounded-xl border border-slate-300 px-3 py-2.5 outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-100" />
                <input required name="reason" placeholder="Motivo del ajuste" aria-label={`Motivo del ajuste para ${item.product.name}`} className="min-w-0 rounded-xl border border-slate-300 px-3 py-2.5 outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-100" />
                <button className="rounded-xl bg-blue-700 px-4 py-2.5 font-bold text-white hover:bg-blue-800">Ajustar</button>
              </form>
            </article>
          );
        })}

        {!items.length && <div className="rounded-3xl border border-dashed border-slate-300 bg-white px-6 py-14 text-center text-slate-500">Todavía no hay registros de inventario.</div>}
      </div>
      ...
      {historyItem && (
        <InventoryHistoryModal
          inventory={{ productName: historyItem.product.name, sku: historyItem.product.sku, location: historyItem.location.name }}
