...
export default async function AdminPage() {
  await requireStaff();
  const [sales, orders, customers, vehicles, lowStock, recent] = await Promise.all([
  ...
  const cards = [
    ["Ventas pagadas", formatPrice(sales._sum.total ?? 0)],
    ["Pedidos", orders],
    ["Clientes", customers],
  ...
  return (
    <main className="mx-auto max-w-7xl px-6 py-10">
      ...
      <section className="mt-8 rounded-3xl bg-white p-7">
        <h2 className="text-2xl font-black">Actividad administrativa</h2>
        {recent.length ? (
          <ul className="mt-4 divide-y">
            {recent.map((log) => (
              <li key={log.id} className="py-3 text-sm">
                <b>{log.action}</b> en {log.entity} · {log.user?.email ?? "sistema"}
                <span className="float-right text-slate-500">{log.createdAt.toLocaleString("es-CL")}</span>
              </li>
            ))}
          </ul>
        ) : (
          <p className="mt-4 text-slate-500">Aún no hay actividad registrada.</p>
        )}
      </section>
    </main>
  );
