...
const errors: Record<string, string> = {
  ...
  duplicate: "Ya existe otra importación con ese VIN.",
};
const hazmatOptions = [
...
export default async function EditImportPage({
  ...
  const [item, customers, makes, query] = await Promise.all([
    db.vehicleImport.findUnique({
      where: { id },
      include: {
        makeRecord: { select: { id: true, name: true } },
        modelRecord: { select: { id: true, name: true } },
        parts: {
  ...
  const initialMake =
    item.makeRecord ??
    (item.make
      ? makes.find((make) => make.name.localeCompare(item.make!, "es", { sensitivity: "base" }) === 0)
      : null) ??
    null;
  const initialModel =
    item.modelRecord ??
    (initialMake && item.model
      ? await db.vehicleModel.findFirst({
          where: { makeId: initialMake.id, name: { equals: item.model, mode: "insensitive" } },
          select: { id: true, name: true },
        })
      : null);
  const title =
    item.importType === "PARTS"
      ? `Repuestos · ${item.referenceCode}`
      : `${item.year} ${item.make} ${item.model} · VIN ${item.vin}`;
  return (
    <main className="mx-auto max-w-6xl px-6 py-10">
      <Link href={`/admin/imports/${item.id}`} className="text-sm font-bold text-blue-700">
        ← Cancelar y volver
      </Link>
      <h1 className="mt-5 text-4xl font-black">Editar importación</h1>
      <p className="mt-2 text-slate-600">{title}</p>
      {query.error && (
        <p role="alert" className="mt-5 rounded-2xl bg-red-50 p-4 font-semibold text-red-700">
          {errors[query.error] ?? errors.invalid}
        </p>
      )}
      <form action={updateVehicleImportAction} className="mt-8 grid gap-6">
        ...
        <section className="grid gap-4 rounded-3xl bg-white p-6 shadow-sm sm:grid-cols-2 lg:grid-cols-3">
          ...
          <CustomerCombobox customers={customers} initialCustomerId={item.customerId} />
          <label className={label}>
            N.º de lote {optional}
            <input name="lotNumber" defaultValue={item.lotNumber ?? ""} className={field} />
          </label>
          <label className={label}>
            Tipo de carga {optional}
            <input name="loadType" defaultValue={item.loadType ?? ""} className={field} />
          </label>
          <label className={`${label} sm:col-span-2`}>
            Puerto o destino {optional}
            <input name="destinationPort" defaultValue={item.destinationPort ?? ""} className={field} />
          </label>
          <ImportDatePicker initialValue={item.receivedDate?.toISOString().slice(0, 10) ?? ""} />
          <StyledSelect
            name="hazmat"
            label="Carga peligrosa (HAZMAT)"
            defaultValue={item.hazmat ? "yes" : "no"}
        ...
        <ImportTypeFields
          lockType
          makes={makes}
          initialType={item.importType as "VEHICLE" | "PARTS"}
          initialVehicle={{
            vin: item.vin ?? "",
            ...
            scheduleB: item.scheduleB,
            make: initialMake,
            model: initialModel,
          }}
          initialParts={item.parts.map((part) => ({
            id: part.id,
            description: part.description,
            partNumber: part.partNumber,
            partBrand: part.partBrand ?? "",
            quantity: part.quantity,
            unitValueUsd: part.unitValueUsd.toString(),
            weightKg: part.weightKg?.toString() ?? "",
            makeId: part.makeId,
          ...
          initialFinance={{
            ...
            towingCostUsd: item.towingCostUsd.toString(),
            oceanFreightUsd: item.oceanFreightUsd.toString(),
            shippingCostUsd: item.shippingCostUsd.toString(),
            logisticsServiceUsd: item.logisticsServiceUsd.toString(),
            otherChargesUsd: item.otherChargesUsd.toString(),
            paidAmountUsd: item.paidAmountUsd.toString(),
          }}
        />
        <section className="grid gap-4 rounded-3xl bg-white p-6 shadow-sm sm:grid-cols-2 lg:grid-cols-3">
          ...
          <label className={label}>
            ...
            <input name="containerNumber" defaultValue={item.containerNumber ?? ""} className={field} />
          </label>
          <label className={label}>
            Naviera o transportista {optional}
            <input name="shippingLine" defaultValue={item.shippingLine ?? ""} className={field} />
          </label>
          <label className={label}>
            Lugar de llegada {optional}
            <input name="arrivalPlace" defaultValue={item.arrivalPlace ?? "Iquique, Chile"} className={field} />
          </label>
          <ImportDatePicker
            name="departureDate"
            label="Fecha de embarque"
            initialValue={item.departureDate?.toISOString().slice(0, 10) ?? ""}
          />
          <ImportDatePicker
            name="arrivalDate"
            label="Fecha de llegada"
            initialValue={item.arrivalDate?.toISOString().slice(0, 10) ?? ""}
          />
        </section>
        <div className="flex flex-wrap justify-end gap-3">
          <Link
            href={`/admin/imports/${item.id}`}
            className="rounded-2xl border bg-white px-6 py-4 font-bold text-slate-700"
          >
            Cancelar
          </Link>
