...
import { CustomerCombobox, ImportDatePicker, ImportVehicleSelector, StyledSelect } from "@/components";
import { requireStaff } from "@/lib/auth";
import { db } from "@/lib/db";
import { importStatusSteps } from "@/lib/import-status";

export const dynamic = "force-dynamic";
const errors: Record<string, string> = { invalid: "Revisa los datos obligatorios.", customer: "Selecciona un cliente válido.", duplicate: "Ya existe otra importación con ese VIN." };
...
export default async function EditImportPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  return <main className="mx-auto max-w-6xl px-6 py-10"><Link href={`/admin/imports/${item.id}`} className="text-sm font-bold text-blue-700">← Cancelar y volver</Link><h1 className="mt-5 text-4xl font-black">Editar importación</h1><p className="mt-2 text-slate-600">{item.year} {item.make} {item.model} · VIN {item.vin}</p>
    ...
    <form action={updateVehicleImportAction} className="mt-8 grid gap-6"><input type="hidden" name="id" value={item.id}/>
      ...
      <section className="grid gap-4 rounded-3xl bg-white p-6 shadow-sm sm:grid-cols-2"><div className="sm:col-span-2"><div className="flex items-center gap-3"><h2 className="text-xl font-black">Participantes</h2><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-600">Todos opcionales</span></div></div><label className={label}>Cliente (referencia documental) {optional}<input name="customerParty" defaultValue={item.customerParty ?? ""} className={field}/></label><label className={label}>Embarcador {optional}<input name="shipper" defaultValue={item.shipper ?? ""} className={field}/></label><label className={label}>Consignatario {optional}<input name="consignee" defaultValue={item.consignee ?? ""} className={field}/></label><label className={label}>Parte a notificar {optional}<input name="notifyParty" defaultValue={item.notifyParty ?? ""} className={field}/></label><label className={`${label} sm:col-span-2`}>Referencia de exportación {optional}<input name="exportReference" defaultValue={item.exportReference ?? ""} className={field}/></label></section>
