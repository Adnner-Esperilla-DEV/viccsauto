    ...
    <main className="mx-auto max-w-7xl px-6 py-10">
      ...
      {ok && (
        <p className="mt-6 rounded-2xl bg-emerald-50 p-4 font-bold text-emerald-800">
          Venta {ok} registrada correctamente. El stock ya fue actualizado.
        </p>
      )}

      <div className="mt-8">
        <PosSaleForm products={products} />
      </div>
      ...
      <section className="mt-10 rounded-3xl bg-white p-6 shadow-sm">
        ...
        {recentSales.length ? (
              ...
              <article key={sale.id} className="flex flex-wrap items-center gap-4 py-4">
                <div className="mr-auto">
                  ...
                  <small className="block text-slate-500">
                    {sale.customerName} · {sale.createdAt.toLocaleString("es-CL")}
                  </small>
                </div>
                <span className="rounded-full bg-emerald-50 px-3 py-1 text-sm font-bold text-emerald-700">
                  {sale.payments[0]?.provider.replace("POS_", "") ?? "PAGADO"}
                </span>
          ...
          <p className="mt-4 text-slate-500">Aún no hay ventas presenciales registradas.</p>
        )}
      </section>
    </main>
  );
