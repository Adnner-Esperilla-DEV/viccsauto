...
async function audit(userId: string, action: string, entity: string, entityId: string, details?: unknown) {
  ...
  await db.auditLog.create({
    data: { userId, action, entity, entityId, details: details ? JSON.stringify(details) : null, ip },
  });
}
...
function parseImageReferences(rawValue: string, maxImages: number, allowExisting: boolean): ImageReference[] | null {
  try {
    ...
    if (
      !value.every((item): item is ImageReference => {
        ...
        const hasData =
          ...
          reference.data.length <= 145_000 &&
          /^data:image\/webp;base64,[A-Za-z0-9+/=]+$/.test(reference.data);
...
function compatibilityDestination(formData: FormData, productId: string, result: { error?: string; ok?: string }) {
  ...
  const query = String(formData.get("listQuery") ?? "")
    .trim()
    .slice(0, 100);
...
export async function createCategoryAction(formData: FormData) {
  ...
  const parsed = z
    .object({
      ...
      icon: z.enum(["brake", "engine", "light", "battery", "body"]),
    })
  ...
  redirect("/admin/catalog?ok=category");
}
...
export async function createBrandAction(formData: FormData) {
  ...
  const row = await db.brand.upsert({
    ...
    update: { slug: parsed.data.slug, isActive: true },
    create: parsed.data,
  });
  await audit(user.id, "UPSERT", "Brand", row.id, parsed.data);
  revalidatePath("/admin/catalog");
  redirect("/admin/catalog?ok=brand");
}
...
export async function createProductAction(formData: FormData) {
  ...
  const parsed = z
    ...
    .refine(hasCompleteCurrencyReference, { path: ["usdToClpRate"] })
    .safeParse(Object.fromEntries(formData));
  ...
  try {
    ...
    if (
      ...
      !value.every(
        (item) =>
          typeof item === "string" && item.length <= 145_000 && /^data:image\/webp;base64,[A-Za-z0-9+/=]+$/.test(item),
      )
    ) {
      ...
      if (uploadedImages.length)
        await tx.productImage.createMany({
          data: uploadedImages.map((image, position) => ({
            productId: row.id,
            ...image,
            alt: `${row.name} - imagen ${position + 1}`,
            position,
          })),
        });
...
export async function updateProductAction(formData: FormData) {
  ...
  const destination = (suffix: string) =>
    returnTo === "list"
  ...
  const current = await db.product.findUnique({
    ...
    include: { images: { select: { id: true, storageKey: true } } },
  });
  ...
  try {
      ...
      for (const [position, image] of imageReferences.entries()) {
        if (image.id) {
          await tx.productImage.updateMany({
            ...
            data: { position, alt: `${data.name} - imagen ${position + 1}` },
          });
        } else if (image.data) {
          const uploaded = uploadedImages[uploadedIndex++];
          await tx.productImage.create({
            data: { productId: id, ...uploaded, alt: `${data.name} - imagen ${position + 1}`, position },
          });
        }
      }
    });
    ...
    await deleteObjectsBestEffort(uploadedImages.map((image) => image.storageKey));
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002")
      redirect(destination("error=product-duplicate"));
    throw error;
  }
...
export async function addProductCompatibilityAction(formData: FormData) {
  ...
  const parsed = z
    .object({
      ...
      yearTo: z.coerce.number().int().min(1950).max(2100),
      engine: z.string().trim().max(100).optional(),
      notes: z.string().trim().max(300).optional(),
    })
    .refine((value) => value.yearTo >= value.yearFrom, { path: ["yearTo"] })
    .safeParse(Object.fromEntries(formData));
  ...
  if (!product || !vehicleModel || !startYear || !endYear)
    redirect(compatibilityDestination(formData, parsed.data.productId, { error: "compatibility" }));
  ...
  if (!duplicate) {
    const row = await db.productCompatibility.create({
      data: {
        productId: product.id,
        vehicleModelId: vehicleModel.id,
        make: vehicleModel.make.name,
    ...
    await audit(user.id, "CREATE", "ProductCompatibility", row.id, {
      productId: product.id,
      vehicleModelId: vehicleModel.id,
...
export async function removeProductCompatibilityAction(formData: FormData) {
  ...
  const row = await db.productCompatibility.findFirst({
...
export async function createVehicleAction(formData: FormData) {
  ...
  try {
      ...
      if (uploadedImages.length)
        await tx.vehicleImage.createMany({
          data: uploadedImages.map((image, position) => ({
            ...
            ...image,
            alt: `${vehicle.year} ${vehicle.make} ${vehicle.model} - imagen ${position + 1}`,
            position,
          })),
        });
      return vehicle;
    ...
    await deleteObjectsBestEffort(uploadedImages.map((image) => image.storageKey));
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002")
      redirect("/admin/vehicles?error=vehicle-duplicate");
    throw error;
  }
...
export async function updateVehicleAction(formData: FormData) {
  ...
  try {
    ...
    await deleteObjectsBestEffort(uploadedImages.map((image) => image.storageKey));
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002")
      redirect(destination("error=vehicle-duplicate"));
    throw error;
  }
  await deleteObjectsBestEffort(removedImages.map((image) => image.storageKey));
  await audit(user.id, "UPDATE", "Vehicle", id, {
...
export async function updateVehicleStatusAction(formData: FormData) {
  ...
  const pageParam = returnPage > 1 ? `page=${returnPage}&` : "";
  const parsed = z
    .object({ id: z.string(), status: z.enum(["AVAILABLE", "RESERVED", "SOLD", "INACTIVE"]) })
    .safeParse(Object.fromEntries(formData));
...
export async function adjustInventoryAction(formData: FormData) {
  ...
  await db.$transaction(async (tx) => {
    await tx.inventoryItem.update({ where: { id: item.id }, data: { quantity: { increment: parsed.data.delta } } });
    await tx.product.update({ where: { id: item.productId }, data: { stock: { increment: parsed.data.delta } } });
    await tx.inventoryMovement.create({
      data: {
  ...
  revalidatePath("/admin/inventory");
  redirect(destination("ok=adjusted"));
}
...
export async function updateOrderAction(formData: FormData) {
  ...
  const parsed = z
    .object({
      ...
      fulfillmentStatus: z.enum(["UNFULFILLED", "READY", "SHIPPED", "DELIVERED", "CANCELLED"]),
  ...
  await db.$transaction(async (tx) => {
    ...
    await tx.payment.updateMany({ where: { orderId: current.id }, data: { status: parsed.data.paymentStatus } });
    await tx.shipment.updateMany({
      where: { orderId: current.id },
      data: {
        status: parsed.data.fulfillmentStatus,
        trackingCode: parsed.data.trackingCode || undefined,
        shippedAt: parsed.data.fulfillmentStatus === "SHIPPED" ? new Date() : undefined,
        deliveredAt: parsed.data.fulfillmentStatus === "DELIVERED" ? new Date() : undefined,
      },
    });
  });
  await audit(user.id, "STATUS_CHANGE", "Order", current.id, parsed.data);
  revalidatePath("/admin/orders");
  redirect(destination("ok=updated"));
}
