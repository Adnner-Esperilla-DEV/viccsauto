...
const saleSchema = z.object({
  items: z
    ...
    .pipe(z.array(saleItem).min(1).max(100))
    .refine(
  ...
  customerPhone: z
    ...
    .trim()
    .max(30)
    .refine((value) => !value || (normalizeCustomerPhone(value)?.length ?? 0) >= 8, "Teléfono inválido"),
        ...
        const order = await tx.order.create({
          data: {
            ...
            shippingAddress: JSON.stringify({
              type: "POS",
              label: "Venta presencial en Arica",
              city: "Arica",
              region: "Arica y Parinacota",
              destinationZone: "PICKUP_ARICA",
              taxRateBps: 0,
              shippingPayment: "NONE",
              locations: [...new Set(lines.map((line) => line.stockItem.location.name))],
            }),
