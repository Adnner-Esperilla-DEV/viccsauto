...
const importDataSchema = z
  ...
  .superRefine((value, context) => {
    if (value.importType === "VEHICLE") {
      if (!value.vin || value.vin.length < 11)
        context.addIssue({ code: "custom", path: ["vin"], message: "VIN obligatorio." });
      if (!value.year || value.year < 1900 || value.year > 2100)
        context.addIssue({ code: "custom", path: ["year"], message: "Año obligatorio." });
      ...
      if (!value.color || value.color.length < 2)
        context.addIssue({ code: "custom", path: ["color"], message: "Color obligatorio." });
    }
    if (value.departureDate && value.arrivalDate && value.arrivalDate < value.departureDate) {
      context.addIssue({
        code: "custom",
        path: ["arrivalDate"],
        message: "La fecha de llegada no puede ser anterior a la fecha de embarque.",
      });
    }
  });
...
const importedPartSchema = z
  ...
  .refine((part) => part.yearTo >= part.yearFrom, {
    path: ["yearTo"],
    message: "El año final no puede ser menor al inicial.",
  });

function parseParts(formData: FormData) {
  try {
    return z
      .array(importedPartSchema)
      .min(1)
      .max(50)
      .safeParse(JSON.parse(String(formData.get("partsJson") ?? "[]")));
  } catch {
...
async function audit(userId: string, action: string, entityId: string, details?: unknown) {
  ...
  await db.auditLog.create({
    data: { userId, action, entity: "VehicleImport", entityId, details: details ? JSON.stringify(details) : null, ip },
  });
}
...
async function partsHaveValidCompatibility(parts: z.infer<typeof importedPartSchema>[]) {
  ...
  const models = await db.vehicleModel.findMany({
    where: { id: { in: modelIds }, isActive: true, make: { isActive: true } },
...
export async function createVehicleImportAction(formData: FormData) {
  ...
  const noteResult = z
    .preprocess((value) => (value === "" || value == null ? undefined : value), z.string().trim().max(2000).optional())
    .safeParse(formData.get("initialNote"));
  ...
  const [customer, selectedVehicleModel, validParts] = await Promise.all([
    db.user.findFirst({
      where: { id: parsed.data.customerId, role: "CUSTOMER", status: { in: ["ACTIVE", "POS_ONLY"] } },
      select: { id: true },
    }),
    parsed.data.importType === "VEHICLE"
      ? db.vehicleModel.findFirst({
          where: { id: parsed.data.modelId, makeId: parsed.data.makeId, isActive: true, make: { isActive: true } },
  ...
  const goodsValueUsd =
    parts.reduce((totalCents, part) => totalCents + Math.round(part.unitValueUsd * 100) * part.quantity, 0) / 100;
  const finance = getImportFinanceSummary({
    importType: parsed.data.importType,
    valueUsd: parsed.data.importType === "PARTS" ? goodsValueUsd : parsed.data.valueUsd,
    towingCostUsd: parsed.data.towingCostUsd,
    oceanFreightUsd: parsed.data.oceanFreightUsd,
    shippingCostUsd: parsed.data.shippingCostUsd,
    logisticsServiceUsd: parsed.data.logisticsServiceUsd,
    otherChargesUsd: parsed.data.otherChargesUsd,
  ...
  if (
    ...
    attachmentFiles.some((file) => file.size > 5 * 1024 * 1024 || !allowedAttachmentTypes.has(file.type)) ||
    attachmentFiles.reduce((sum, file) => sum + file.size, 0) > 20 * 1024 * 1024
  )
    importError("attachments");
  ...
  try {
    ...
    vehicleImport = await db.$transaction(async (tx) => {
      const row = await tx.vehicleImport.create({
        data: {
          importType: data.importType,
          referenceCode: newReferenceCode(),
          customerId: data.customerId,
          vin: data.importType === "VEHICLE" ? data.vin : null,
          year: data.importType === "VEHICLE" ? data.year : null,
          makeId: data.importType === "VEHICLE" ? data.makeId : null,
          make: data.importType === "VEHICLE" ? selectedVehicleModel?.make.name : null,
          modelId: data.importType === "VEHICLE" ? data.modelId : null,
          model: data.importType === "VEHICLE" ? selectedVehicleModel?.name : null,
          color: data.importType === "VEHICLE" ? data.color : null,
          fuel: data.importType === "VEHICLE" ? data.fuel : null,
          weightKg: data.importType === "VEHICLE" ? data.weightKg : null,
          valueUsd: data.importType === "PARTS" ? goodsValueUsd : (data.valueUsd ?? null),
          towingCostUsd: data.importType === "VEHICLE" ? data.towingCostUsd : 0,
          oceanFreightUsd: data.importType === "VEHICLE" ? data.oceanFreightUsd : 0,
          shippingCostUsd: data.importType === "PARTS" ? data.shippingCostUsd : 0,
          logisticsServiceUsd: data.importType === "PARTS" ? data.logisticsServiceUsd : 0,
          ...
          hazmat: data.hazmat,
          keyStatus: data.importType === "VEHICLE" ? data.keyStatus : "UNKNOWN",
          titleStatus: data.importType === "VEHICLE" ? data.titleStatus : "PENDING",
          titleNumber: data.importType === "VEHICLE" ? data.titleNumber : null,
          titleState: data.importType === "VEHICLE" ? data.titleState : null,
          scheduleB: data.importType === "VEHICLE" ? data.scheduleB : null,
      ...
      if (parts.length)
        await tx.importedPart.createMany({
          data: parts.map((part, position) => ({
            ...
            unitValueUsd: part.unitValueUsd,
      ...
      await tx.vehicleImportImage.createMany({
        data: uploadedImages.map((image, position) => ({
          importId: row.id,
      ...
      if (uploadedAttachments.length)
        await tx.vehicleImportAttachment.createMany({
          data: uploadedAttachments.map((attachment) => ({
            importId: row.id,
            filename: attachment.filename,
            mimeType: attachment.mimeType,
            size: attachment.size ?? attachment.data.length,
            storageKey: attachment.storageKey,
          })),
        });
      if (initialNote)
        await tx.vehicleImportNote.create({
          data: { importId: row.id, authorId: user.id, body: initialNote, visibleToCustomer: true },
        });
      return row;
    });
  } catch (error) {
  ...
  await audit(user.id, "CREATE", vehicleImport.id, {
    importType: vehicleImport.importType,
    referenceCode: vehicleImport.referenceCode,
    vin: vehicleImport.vin,
...
export async function updateVehicleImportAction(formData: FormData) {
  ...
  const [existing, customer, selectedVehicleModel, validParts] = await Promise.all([
    ...
    db.user.findFirst({
      where: { id: data.customerId, role: "CUSTOMER", status: { in: ["ACTIVE", "POS_ONLY"] } },
      select: { id: true },
    }),
    data.importType === "VEHICLE"
      ? db.vehicleModel.findFirst({
          where: { id: data.modelId, makeId: data.makeId, isActive: true, make: { isActive: true } },
          select: { name: true, make: { select: { name: true } } },
        })
      : null,
  ...
  const finance = getImportFinanceSummary({
    importType: data.importType,
    valueUsd: data.importType === "PARTS" ? goodsValueUsd : data.valueUsd,
    towingCostUsd: data.towingCostUsd,
    oceanFreightUsd: data.oceanFreightUsd,
    shippingCostUsd: data.shippingCostUsd,
    logisticsServiceUsd: data.logisticsServiceUsd,
    otherChargesUsd: data.otherChargesUsd,
    paidAmountUsd: data.paidAmountUsd,
  });
  if (Math.round(finance.paidAmountUsd * 100) > Math.round(finance.totalUsd * 100))
    redirect(`/admin/imports/${data.id}/edit?error=payment`);
  const { id } = data;
  try {
    await db.$transaction(async (tx) => {
      await tx.vehicleImport.update({
        ...
        data: {
          ...
          keyStatus: data.importType === "VEHICLE" ? data.keyStatus : "UNKNOWN",
          titleStatus: data.importType === "VEHICLE" ? data.titleStatus : "PENDING",
          titleNumber: data.importType === "VEHICLE" ? (data.titleNumber ?? null) : null,
          titleState: data.importType === "VEHICLE" ? (data.titleState ?? null) : null,
          scheduleB: data.importType === "VEHICLE" ? (data.scheduleB ?? null) : null,
          containerNumber: data.containerNumber ?? null,
          shippingLine: data.shippingLine ?? null,
          departureDate: data.departureDate ?? null,
          arrivalDate: data.arrivalDate ?? null,
          arrivalPlace: data.arrivalPlace ?? null,
          status: data.status,
        },
      });
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002")
      redirect(`/admin/imports/${id}/edit?error=duplicate`);
    throw error;
  }
  await audit(user.id, "UPDATE", id, {
    importType: data.importType,
...
export async function addVehicleImportImagesAction(formData: FormData) {
  ...
  try {
    await db.vehicleImportImage.createMany({
...
export async function deleteVehicleImportImageAction(formData: FormData) {
  const user = await requireStaff();
  const parsed = z
    .object({ importId: z.string().min(1), imageId: z.string().min(1) })
    .safeParse(Object.fromEntries(formData));
  if (!parsed.success) redirect("/admin/imports");
  const image = await db.vehicleImportImage.findFirst({
    where: { id: parsed.data.imageId, importId: parsed.data.importId },
    select: { id: true, importId: true, filename: true, storageKey: true },
  });
...
export async function addVehicleImportAttachmentsAction(formData: FormData) {
  ...
  if (
    !files.length ||
    files.length > 5 ||
    files.some((file) => file.size > 5 * 1024 * 1024 || !allowedAttachmentTypes.has(file.type)) ||
    files.reduce((sum, file) => sum + file.size, 0) > 20 * 1024 * 1024
  )
    redirect(`/admin/imports/${exists.id}?error=attachments`);
  const attachments = await Promise.all(
  ...
  try {
    await db.vehicleImportAttachment.createMany({
      data: uploadedAttachments.map((attachment) => ({
        importId: exists.id,
        filename: attachment.filename,
        mimeType: attachment.mimeType,
  ...
  await audit(user.id, "ADD_ATTACHMENTS", exists.id, {
...
export async function deleteVehicleImportAttachmentAction(formData: FormData) {
  const user = await requireStaff();
  const parsed = z
    .object({ importId: z.string().min(1), attachmentId: z.string().min(1) })
    .safeParse(Object.fromEntries(formData));
  if (!parsed.success) redirect("/admin/imports");
  const attachment = await db.vehicleImportAttachment.findFirst({
    where: { id: parsed.data.attachmentId, importId: parsed.data.importId },
    select: { id: true, importId: true, filename: true, storageKey: true },
  });
  ...
  await audit(user.id, "DELETE_ATTACHMENT", attachment.importId, {
...
export async function updateVehicleImportStatusAction(formData: FormData) {
  const user = await requireStaff();
  const parsed = z
    .object({ id: z.string().min(1), status: importStatusSchema })
    .safeParse(Object.fromEntries(formData));
...
export async function addVehicleImportNoteAction(formData: FormData) {
  ...
  if (!exists) redirect("/admin/imports");
  const note = await db.vehicleImportNote.create({
    data: {
      importId: exists.id,
      authorId: user.id,
