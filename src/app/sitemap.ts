...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const [categories, products, vehicles] = await Promise.all([
    ...
    db.vehicle.findMany({ where: { status: "AVAILABLE" }, select: { slug: true, updatedAt: true } }),
  ]);
  const now = new Date();
  ...
  return [
    ...staticPages,
    ...categories.map((row) => ({
      url: `${siteConfig.url}/category/${row.slug}`,
      lastModified: row.updatedAt,
      changeFrequency: "weekly" as const,
      priority: 0.7,
    })),
    ...products.map((row) => ({
      url: `${siteConfig.url}/product/${row.slug}`,
      lastModified: row.updatedAt,
      changeFrequency: "weekly" as const,
      priority: 0.8,
    })),
    ...vehicles.map((row) => ({
      url: `${siteConfig.url}/vehicle/${row.slug}`,
      lastModified: row.updatedAt,
